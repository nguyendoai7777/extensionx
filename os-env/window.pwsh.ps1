...
function grho {
  param([string]$cm)
  process { git reset --hard origin/$cm }
}
...
function gstp {
  param([string]$order)
  process { git stash pop "stash@{$mess}" }
}

function gsts {
  param([string]$order)
  process { git stash show "stash@{$mess}" }
}

function gsta {
  param([string]$order)
  process { git stash apply "stash@{$mess}" }
}
...
function grbi {
  ...
  process { git rebase -i $command }
}

function grb {
  param([string]$branch)
  process { git rebase $branch }
}
...
function gplo {
  param([string]$brName)
  process { git pull origin $brName }
}  
...
function gnb {
  param([string]$cm)
  process { git checkout -b $cm }
}

function gco {
  param([string]$br)
  process { git checkout $br }
} 

function gcl {
  param([string]$brName)
  process { git clone $brName }
}  
...
function gcp { 
  param([string]$branch)
  process { git cherry-pick $branch }
}   
...
function gmg {
  param([string]$branch)
  process { git merge $branch }
}

function gcmm {
  param([string]$cm)
  process { git commit -m $cm }
}   
