...
function startRipple(event, el, { duration = 400, zIndex = '9999', bgColor = 'currentColor', maxOpacity = '0.4', easing = 'cubic-bezier(0.4, 0, 0.2, 1)' } = {}) {
	...
	let storedTargetPosition = target.style.position.length > 0 ? target.style.position : window.getComputedStyle(target).position;
	...
	let releaseEvent = eventType === 'mousedown' ? 'mouseup' : eventType === 'touchstart' ? 'touchend' : false;
	if (releaseEvent) {
